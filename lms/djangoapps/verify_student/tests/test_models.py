...
class SkippedReverificationTest(ModuleStoreTestCase):
    ...
    def test_add_skipped_attempts(self):
        ...
        SkippedReverification.add_skipped_reverification_attempt(
            checkpoint=self.checkpoint, user_id=self.user.id, course_id=self.course.id
        )
    ...
    def test_unique_constraint(self):
        """adding skipped re-verification with same user and course id not allowed due to unique-together constraint
        """
        ...
        self.assertEqual(1, SkippedReverification.objects.filter(user=self.user, course_id=self.course.id).count())

        # Create skipped attempt for different user
        user2 = UserFactory.create()
        SkippedReverification.add_skipped_reverification_attempt(
            checkpoint=self.checkpoint, user_id=user2.id, course_id=self.course.id
        )
    ...
    def test_check_user_skipped_reverification_exists(self):
        ...
        self.assertTrue(
            SkippedReverification.check_user_skipped_reverification_exists(
                course_id=self.course.id, user_id=self.user.id
            )
        )

        user2 = UserFactory.create()
        self.assertFalse(
            SkippedReverification.check_user_skipped_reverification_exists(course_id=self.course.id, user_id=user2.id)
        )
