...
class TestReportMixin(object):
    ...
    def verify_rows_in_csv(self, expected_rows, verify_order=True, sub_dict_match=False):
        """
        ...
        with open(report_store.path_to(self.course.id, report_csv_filename)) as csv_file:
            ...
            if sub_dict_match:
                csv_rows = [sub_dict(row, expected_rows[index].keys()) for index, row in enumerate(csv_rows)]
